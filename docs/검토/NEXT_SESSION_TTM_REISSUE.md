...
1. **미재실행 13개 결정론 태그 재실행** — `scripts/run_ablation.py --det-only`(반기, `--calendar` 기본값).
   대상은 `run_at < 2026-07-27` 인 것 전부:
...
2. **채택안 재확정** — 수정된 수치로 태그 순위를 다시 매기고, `F_pbr_no_r3r4`가 유지되는지 판정.
   **바뀌면 즉시 멈추고 보고할 것** (SPEC_09~13 상위 전제 재검토가 필요해진다).
3. **SPEC_10 반기 대조군 재실행** — `U_pbr_path_ew`(위 1에 포함) + `C_pbr_path_random` 1,000회
   (`scripts/robustness/run_random_pool.py`, 반기). 등가성 게이트 3종 통과 필수.
4. **SPEC_10 G1/G2/G5 재판정** — `scripts/robustness/gate_analysis.py`(반기 전용, 그대로 사용 가능).
   기존 판정(G1·G2 PASS, G5 FAIL −54.2%)이 유지되는지 확인.
5. **문서 갱신** — SPEC_13 §9-8 해소 기록, SPEC_10/SPEC_11 공표 수치 정정, MASTER.md 표.
   메모리 `pbr_momentum_ablation_results.md`·`backtest_reproducibility_drift.md`도 정정 필요
   (16.19%/16.28% 등이 버그 기준 수치라는 표시).
