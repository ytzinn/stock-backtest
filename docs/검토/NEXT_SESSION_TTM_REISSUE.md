...
> `[정정 2026-08-17]` 여기 적힌 **수치는 계정 표준화 결함 이전** 기준이라 폐기한다.
> 다만 아래 완료 배너의 **`F_pbr_no_r2r3r4` 기각 사유("1구간 1종목 마진")는 결함 수정 후
> 재실행에서도 동일하게 재현됐다.** 그 기각은 `docs/검토/REJECTED.md` 로 옮겨 적었다 —
> 소진된 인계 문서에 묻혀 있어 다음 세션이 같은 실험을 다시 돌렸기 때문이다.

> ## ✅ `[완료 2026-07-30]` 이 인계는 소진됐다 — 결과는 SPEC_13 §9-9
> 61개 태그 전부 재실행, SPEC_10 G1/G2/G5 재판정, 문서 정정까지 완료. 산출물 커밋 `05d8fc7`.
> **결론: 채택안 교체 없음**(1위는 그리드서치라 자격 없음, `F_pbr_ma200`은 G5 악화,
> `F_pbr_no_r2r3r4`는 21구간 중 1구간 1종목 마진). SPEC_10 판정도 불변(G1·G2 PASS, G5 FAIL).
>
> **이 문서의 사실오류 2건**(다음 세션이 같은 함정에 빠지지 않도록 기록):
> 1. "42개 공표 수치가 `summary.json`에만 존재" → **사실 아님.** 그 파일은 07-20 이후 계속
>    1개 시나리오뿐이었다. `run_ablation`의 전체 덮어쓰기 버그로 이미 유실된 상태였고,
>    07-18 공표본(39태그)은 커밋 `d139057`에만 남아 있었다.
> 2. "`summary*.json`은 이제 태그 단위로 병합된다" → **`run_daily_nav.py`에만 적용**돼 있었다.
>    `run_ablation.py`는 누락 → 커밋 `0d6ee86`에서 동일 패턴으로 수정.
>
> 아래 원문은 당시 판단의 기록으로 보존한다.
...
동일 동결 스냅샷에서 수정 직전 코드(`ed2856e`)로 현행안을 재실행해 측정한 결과:
...
미뤘는데, 프로즌 단계(Q-C2)에서 **현행안 1개만** 재고정하고 범위를 명시하지 않은 것이 원인이다.
...
**모든 실행은 기존 동결 스냅샷에서** — 그래야 이미 확정된 현행안(14.0799%)·Q-G 대조군과 정합한다.
```
...
3. **현행안 `F_pbr_no_r3r4` 재실행해 net CAGR `0.14079850522450377` 비트 재현 확인** —
