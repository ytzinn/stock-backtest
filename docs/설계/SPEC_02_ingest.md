...
| 항목 | 수치 |
|------|------|
| 대상 종목 수 | ~2,670개 (현재 상장) + 상장폐지 종목 (~1,500개 추정) |
| 수집 시작 연도 | 2014년 (백테스트 2015년 시작 기준 FY 데이터 확보) |
| DART 호출 수 (현재 상장) | 2,500 × 60 ≈ 150,000회 (공시 12 + 재무 48/ticker) |
| DART 일일 한도 | **`[정정 2026-07-27]` ~36,000~40,000회** (status 020 초과 시 즉시 차단) |
| 일일 처리 가능 종목 | ~600개/일 (2026-05 실측: 3일간 1,934건 ÷ 3 ≈ 645건/일) |
| 전체 수집 소요 | 약 5~6일 (3,424건 ÷ 600, 2026-05 실측 기준) |

> **`[정정 2026-07-27]` "10,000회 hard limit"은 자체 모순이었다.** 바로 위·아래 행과 대조하면
> 600종목/일 × 60콜/종목 = **36,000콜/일** — 같은 표 안에서 이미 10,000의 3.6배를 실제로
> 소진하고 있었다. 즉 "10,000회"는 이 표의 다른 실측치들과 애초에 앞뒤가 안 맞는 안전선
> 성격의 숫자였고, "2026-05 실측"이라는 표기와 달리 처리량 로그로 직접 검증된 값이 아니었다.
>
> **재검증 (SPEC_13 Q-A1, 2026-07-27)**: Q1/Q3 전체 유니버스 수집에서 07:13~09:16(123분)
> 동안 1,046종목(종목당 ~38콜) 처리 후 status 020 발생 — **약 40,000콜을 2시간여 만에
> 소진**. 이 값이 2026-05 FY+H1 실측(36,000콜/일)과 같은 자릿수라, **실제 일일 한도는
> 처음부터 ~36,000~40,000회였고 10,000회는 잘못 기재된 안전선 숫자였다**고 결론.
...
> ⚠️ **2026-05 버그 수정 이력**: `delisting_ingest.py`에서 FDR `KRX-DELISTING` 티커 컬럼명을
> `Code`로 잘못 읽어 상폐 종목 전체가 stocks에 삽입되지 않는 버그 발견 → `Symbol`로 수정.
> 버그 수정 전 실행된 `dart_ingest`에서 상폐 종목 ~1,297건의 재무 데이터가 미수집됨.
> 수정 후 cron(`--skip-if-done`)으로 재수집 중 (2026-05-09 기준 진행 중, 약 2~3일 소요 예상).
...
> ⚠️ **2026-05 데이터 공백 — 상장폐지 종목 market_cap_history 없음**:
> `_load_shares()`가 `fdr.StockListing('KRX')` (현재 상장 종목만) 에서 주식수를 가져오므로
> 상폐 종목은 `shares = None` → `if not shares: continue` 로 건너뜀.
> **결과**: `market_cap_history` 행 수(~6.4M)가 `price_history` 행 수(~8.6M)보다 ~2.2M 적음.
...
-- status: 'missing' | 'reported_positive' | 'confirmed_zero'
