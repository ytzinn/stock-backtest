...
> ### ⚠️ `[정정 2026-07-30]` §9·§10의 CAGR 수치는 버그 TTM 기준 — 순위가 바뀌었다
> `CORR-TTM-001`(f1d2935) 이전 코드 산출값이다. 2026-07-30 전체 재발행(SPEC_13 §9-9)에서
> **모멘텀 기준 17개를 포함해 61개 태그 전부 재실행**했고, 그 결과 §9의 결론이 일부 바뀐다:
>
> | 태그 | 재발행 gross | 비고 |
> |---|---|---|
> | `F_pbr_ma60_120` | 16.1154% | §10 그리드 — **§6-2에 따라 채택 불가** |
> | `F_pbr_ma200` | **16.0971%** | §6-1 사전등록 primary. 기존 채택안을 **+0.2775%p 앞섬** |
> | `F_pbr_52w75` | 15.6174% | 기존 INCONCLUSIVE |
> | `F_pbr_no_r3r4` | 15.8196% | 현행안 (구 16.28%) |
> | `F_pbr_absret126` | 13.7580% | |
> | `F_pbr_signcount126` | 12.0881% | |
>
> **배관 양성 대조군(§4-5)은 통과**: `F_pbr_ma_double_adapter` = `F_pbr_no_r3r4` = 15.8196%로
> 설계대로 완전 일치(§9 본문의 "cagr=16.28%로 기존과 일치" 서술은 값만 갱신, 결론 동일).
>
> **`F_pbr_ma200`이 현행안을 앞서지만 채택하지 않는다** — 일별 NAV net CAGR은 14.4498%로
> 현행안(14.0799%) 대비 +0.37%p지만, **MDD(net)가 −54.61% → −62.61%로 8.00%p 악화**해
> SPEC_10 G5(−45% 한계)를 더 크게 위반한다. 상세·최종 결정은 SPEC_13 §9-9.
...
**역할 분리**: `F_pbr_no_r3r4` = **성과 기준선(현행안)** / `F_pbr_ma_double_adapter` = **신규 배관의 양성 대조군**.
...
비악화 =  conflict_rate 가 현행안 대비 1개 구간 이상 증가하지 않음
      AND median log-PBR gap 이 현행안 대비 0.05 이상 악화되지 않음
```
...
필수 출력: coverage / 전체·구간별 탈락률 / 현행안과 탈락집합 Jaccard / **전환행렬**[^21](통과→탈락, 탈락→통과) / top 20 변경 종목 수 / turnover 증가분 / **iso-rejection 진단**(현행안 구간별 중앙 탈락률에 맞춘 임계값 비교 — 파라미터 선택용 아님).
...
| 역할 | 태그 | 정의 |
|---|---|---|
| 성과 기준선(현행안) | `F_pbr_no_r3r4` | 현행 MA double, 레거시 경로 |
| **배관 양성 대조군** | `F_pbr_ma_double_adapter` | 신규 배관 + adapter, 현행안과 완전 일치 필수(§4-5) |
...
`[Claude 의견]` v0.2는 단일 seed permutation 하나를 음성 대조군으로 뒀다. **무작위 점수도 우연히 좋은 종목을 거를 수 있고, 표본이 20구간뿐이라 단일 순열이 현행안을 이겨도 아무 의미가 없다.** 한 번의 추출로 분포를 판정한 셈이다.
...
|---|---|---|---|---|---|---|---|---|
| F_pbr_no_r3r4(현행안) | 15.02% | 0.6329 | −54.22% | — | — | — | — | 39.5% |
...
| threshold | net Sharpe | 현행안 대비 Δ | 방향 |
|---|---|---|---|
...
|---|---|---|
| 현행안(MA double) | 0.762(21구간 중 16) | −0.162 |
| F_pbr_52w75 | 0.476(21구간 중 10) | +0.027 |

52w75는 현행안보다 가치-모멘텀 상충이 크게 완화됨(§1-1의 15/20 상충 문제 상당 부분 해소) —
다만 이 결과가 §9-2의 robust 미충족을 상쇄하지는 못한다.

### 9-4. 최종 판정 (§5-4 3단계)

> `[포인터 2026-08-12]` 아래 판정은 **fail-open 정책(§4-4 `insufficient → 통과`) 하의**
> 2026-07-23 격리 스냅샷 수치 기준이다. 정책을 fail-closed 로 바꾼 뒤의 갱신 수치와
> 판정 재확인은 **§11** 참조 — 결론 5개는 전부 유지되나 `ma200`·`signcount126` 의
> **판정 사유가 바뀌었고**, §9-2 robust 근거 2개 중 1개가 반전됐다.
...
§5-8 상위 게이트 불간섭: 위 판정과 무관하게, 현행안 `F_pbr_no_r3r4` 자체가 이미 SPEC_10 G5
...
전부 현행안보다 daily-net CAGR·Sharpe·MDD가 열위** — 5일 단기 MA는 노이즈에 과민 반응,
...
하나씩 변화. **4개 전부 현행안보다 열위**(Sharpe Δ −0.017~−0.104). `slope_lookback=30`이
가장 근접(Δ −0.017).
...
(현행안−변형)를 10,000회 리샘플링(seed 고정)해 평균차 90%/95% CI와
...
## 11. `[재발행 2026-08-12]` fail-closed 정책 변경 반영

> **§9·§10 원문은 수정하지 않는다.** 사전등록 판정은 "그 규칙 + 그 데이터에서 무엇이
> 나왔나"의 기록이고, 결론만 덮어쓰면 사전등록이 지켜졌는지 검증할 수단이 사라진다.
> 이 절이 갱신본이며, §9-4 판정문에 이리로 오는 포인터를 달았다.

### 11-1. 무엇이 왜 바뀌었나

§4-4가 정한 `insufficient → 통과` 정책을 **`reject`(fail-closed)로 바꿨다**
(`docs/설계/[이슈] 모멘텀필터_coverage_gate_미구현.md`, `on_insufficient` 스위치).

HardFilter는 상장 6개월(약 124거래일)에 유니버스로 넣는데 창이 그보다 긴 criterion 은
그 틈의 종목을 **신호가 좋아서가 아니라 자료가 없어서** 통과시켰다. §9-4 하단에 기록된
버그 수정 ①(신규상장을 `invalid` 아닌 `insufficient` 로 분류)이 바로 이 통로를 연 것이고,
당시 채택안 MA 20/60 은 요구 이력 80거래일 < 124 라 **한 번도 발화하지 않아** 문제가
드러나지 않았다. 2026-08-10 MA200 채택으로 처음 살아났다.

§9 후보 5개 중 노출 정도:

| 태그 | 요구 이력 | insufficient 비율 | 구간 CAGR 변화(동일 DB 대조) |
|---|---:|---:|---:|
| `F_pbr_signcount126` | 148 | — | **+0.62%p** |
| `F_pbr_52w75` | 252 | 2.71% | **+0.29%p** |
| `F_pbr_ma200` | 200 | 1.77% | +0.07%p |
| `F_pbr_mktresid126` | 274 | — | −0.00%p |
| `F_pbr_absret126` | 148 | — | **0.00%p (비트 불변)** |
| 현행안 `F_pbr_no_r3r4` | 80 | **0.00%** | **0.00%p (구조적 미노출)** |

위 "구간 CAGR 변화"는 **동일 DB·동일 코드에서 스위치만 바꾼** 대조라 fail-closed 순효과다.

### 11-2. ⚠️ 이 절의 절대 수치는 §9 와 직접 비교할 수 없다

| | §9 원본 | §11 재산출 |
|---|---|---|
| DB | 격리 스냅샷 5435 (2026-07-23) | **운영 5433 (2026-08-12)** |
| 코드 | 2026-07-23 시점 | master `9c63d0c` + fail-closed |
| 그 사이 변경 | — | CORR-TTM-001 전체 재발행(07-30) 등 |

따라서 §9 표와 아래 표의 차이는 **fail-closed 효과 + 데이터·코드 변경이 섞여 있고
분리되지 않는다.** 순수 fail-closed 효과는 §11-1 마지막 열(동일 DB 대조)만 유효하다.
아래 표는 "§9 대비 얼마나 변했나"가 아니라 **"현재 기준값이 얼마인가"** 로만 읽을 것.

### 11-3. §9-1 재산출 — 일별 net 지표 (현행안 대비)

| 태그 | net CAGR | Sharpe | MDD | ΔSharpe | ΔMDD | ΔCAGR |
|---|---:|---:|---:|---:|---:|---:|
| **`F_pbr_52w75`** | 14.49% | **0.653** | **−50.22%** | **+0.059** | **+4.39%p** | +0.41%p |
| `F_pbr_ma200` | 14.52% | 0.603 | −62.32% | +0.008 | −7.71%p | +0.44%p |
| `F_pbr_no_r3r4` ←현행안 | 14.08% | 0.594 | −54.61% | — | — | — |
| `F_pbr_absret126` | 12.13% | 0.520 | −58.72% | −0.074 | −4.12%p | −1.95%p |
| `F_pbr_mktresid126` | 12.09% | 0.532 | −56.60% | −0.063 | −2.00%p | −1.99%p |
| `F_pbr_signcount126` | 11.02% | 0.492 | −53.22% | −0.102 | +1.38%p | −3.06%p |

§5-3 문턱: Sharpe **≥+0.05** 또는 MDD **≥+2.0%p**, CAGR 열위 **≥−1.0%p**.

### 11-4. §9-2 재산출 — 52w OAT 밴드

| threshold | §9 원본 Δ | §11 재산출 Δ | 방향 |
|---|---:|---:|---|
| 0.70 | −0.046 | −0.0428 | 악화 |
| **0.75 (primary)** | +0.036 | **+0.0586** | 개선 |
| 0.80 | +0.010 | +0.0469 | 개선 |

**판정 `robust 미충족` 유지 — 그러나 두 근거 중 하나가 뒤집혔다.**

| 근거 | §9 원본 | §11 재산출 |
|---|---|---|
| 이웃 같은 방향 비율 | 1/2 = 50% < 70% ❌ | 1/2 = 50% < 70% ❌ **(유지)** |
| 이웃 효과 중앙값 | **−0.018 < 0** ❌ | **+0.0020 > 0** ✅ **(반전)** |

방향 비율 하나만으로 미충족이 유지되므로 판정은 같으나, **근거가 절반으로 줄었다.**
threshold 0.70 에서만 방향이 반대인 knife-edge 패턴은 그대로다.

### 11-5. §9-4 판정 재확인 — **5개 전부 결론 유지**

```
F_pbr_absret126     : FAIL        (유지) Sharpe·MDD 악화 + CAGR 열위 −1.95%p 초과
F_pbr_signcount126  : FAIL        (유지) ※사유 강화 — MDD 개선이 +1.38%p 로 문턱(+2.0) 미달,
                                        CAGR 열위 −3.06%p 초과
F_pbr_ma200         : FAIL        (유지) ※사유 정정 — "Sharpe·MDD 개선 없음"은 부정확.
                                        Sharpe 는 +0.008 로 개선이나 문턱(+0.05) 에 크게 미달,
                                        MDD 는 −7.71%p 악화
F_pbr_52w75         : INCONCLUSIVE(유지) 1차 문턱 충족(Sharpe·MDD 둘 다), robust 미충족
F_pbr_mktresid126   : FAIL        (유지) Sharpe·MDD 악화 + CAGR 열위 −1.99%p 초과
```

§5-8 상위 게이트 불간섭도 그대로다 — 현행 운영안이 이미 SPEC_10 G5 미달이므로 이 판정은
프로덕션 채택 승인이 아니다.

**미재산출 항목**: §9-3 `conflict_rate`(가치-모멘텀 상충)는 이번에 다시 계산하지 않았다.
§5-3 PASS 조건의 보조 항목이고 위 판정을 뒤집을 위치가 아니지만, 정식 재발행 시 포함해야 한다.

### 11-6. `[진단 — 판정 비사용]` 낙폭 축에서의 관찰

미해결 최대 과제가 SPEC_10 G5(일별 net MDD > −45%)인 상황에서 눈에 띄는 값이 있다.

```
F_pbr_52w75    MDD −50.22%   Sharpe 0.653   CAGR 14.49%
F_pbr_ma200    MDD −62.32%   Sharpe 0.603   CAGR 14.52%   ← 현재 운영안
현행안(20/60)  MDD −54.61%   Sharpe 0.594   CAGR 14.08%
```

52w75 는 **CAGR 을 거의 잃지 않으면서(−0.03%p) 낙폭을 12.1%p 얕게** 하고 Sharpe 도 최고다.
G5 문턱 −45% 에 전 태그 중 가장 근접하다.

**그럼에도 채택 후보가 아니다.** ① §11-4 robust 미충족(threshold 0.70 에서 방향 반전),
② SPEC_14 §14-3 시간 축에서 **앞 16위 → 뒤 1위**로 MA200(6·5위)보다 크게 불안정,
③ 반기 캘린더 단독 순위만 보면 MA200 과 근접하나 안 C 에서는 4위 vs 8위로 벌어진다.
**한 축만 보고 판단하면 안 된다** — 이 문서의 반기 표 하나로 교체를 논하지 말 것.

다음 사전등록 대상으로 남긴다. 낙폭 대책이 필요한 것은 사실이나, 근거는 라이브 관측에서
확보해야 한다.

### 11-7. 재현 정보

- 코드: master `9c63d0c` + fail-closed 커밋 `9ff2635`
- DB: 운영 5433, 2026-08-12 (가격 2026-08-10 까지)
- 산출: `experiments/ablation/{tag}.json`, `experiments/daily_nav/{tag}_*`, `summary.json`
- 실행: `run_ablation --det-only` → `export_portfolios` → `run_daily_nav`
...
[^21]: **전환행렬**: 현행안 대비 판정 변화 교차표(통과→탈락, 탈락→통과). 두 신호가 실질적으로 다른 종목을 거르는지 확인.
