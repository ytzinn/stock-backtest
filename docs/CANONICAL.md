...
| `SUMMARY-JSON-STALE` | high | CANONICAL 의 성적·게이트 표가 폐기된 세대의 요약 JSON 에서 생성된다. 2026-08-29 에 막힌 지점이 바뀌었다 — 종전에는 재생성이 등재 기준선을 덮어써서 못 했고, 이제는 워크트리 밖에서 안전하게 재생성해 본 결과 **등재본과 비트 동일하지 않다**는 것을 확인했다. 판정에 쓰이는 컬럼은 전 구간 동일했고 차이는 열린 구간의 실행일 의존과 필터 통과 개수뿐이지만, 승격의 전제("요약 JSON 이 등재 periods.csv 와 같은 상태에서 나왔다")가 성립하지 않으므로 승격하지 않았다. 게이트 표는 등재 산출물에서 조립하는 경로를 만들어 뒀으나, 성적 표가 함께 갱신되지 않으면 한 문서에 두 세대가 섞이므로 반만 실행하지 않았다. 재현 불일치 규명이 선행 조건이다. | `experiments/runs/2026.08.29._PLUMBING.md` |
...
| `MOMENTUM-DRIFT-016600` | medium | 상폐되지 않은 종목 하나의 모멘텀 판정이 두 구간에서 바뀌었다. 같은 코드·같은 섀도우 DB 로 며칠 간격을 두고 tape 을 재생성했을 때 드러났고, 편입 종목은 전 구간 동일해 성과 수치는 움직이지 않았다. 등재 기준선에 이미 반영된 상태다. 모멘텀은 가격 이력에 의존하므로 소스가 과거를 회수하는 중이라면 같은 유형이 더 있을 수 있다 — 데이터 지평 절의 세 번째 축이 활성이라는 신호일 수 있으나 실측하지 않았다. 원인 규명 전에는 재현성 주장을 종목 단위로 하지 마라. 2026-08-29 재현 대조에서 같은 지문이 다시 나왔다 — 수익률은 안 움직이는데 필터 통과 개수만 최근 구간에서 감소했다. 한 건일 가능성이 있으나 확인하지 않았다. | `experiments/runs/2026.08.29._PLUMBING.md` |
...
| `docs/open_issues.yaml` | 5665ed7cf6b6dcc9 |
