...
| `NAME-BASED-TYPE-DETECTION` | medium | 파일·레코드의 종류를 이름·접미사·패턴 매칭으로 판정하는 코드가 남아 있다. 이 저장소에서 같은 유형으로 세 번 틀렸다 — H1 결측 서명 오독, amount 변동을 정정으로 오독, 파일명 접미사로 요약 JSON 계열 오독. 세 번 다 내용 기반 판별자로 고쳐졌다. 신설 조립기 2종은 AST 검사로 이름 기반 판별을 금지했고, 소급 점검은 범위 밖이다. 미점검 지점은 넷이다 — baseline_registry.poisoned_summaries (경로 집합 비교로 조립분과 재오염분을 구분 못 한다), canonical_state.collect 의 키 조립(f-string 태그 이름), dashboard/series.py 의 UNCOVERED 계열 분류, ingest 계열의 account_nm suffix 처리. | `tests/oracle/test_compose_canonical_oracle.py` |
...
| `docs/open_issues.yaml` | 2039c4dfe778746a |
