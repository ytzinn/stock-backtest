...
> **로그 경로**: `/opt/stock-backtest/logs/` (초기 설계의 `/var/log/backtest/`는 사용하지 않음)

```bash
# crontab -e 로 등록 (2026-05-21 수정 기준)
# VENV, WORKDIR 변수는 crontab 상단에 선언
VENV=/opt/stock-backtest/venv/bin/python
WORKDIR=/opt/stock-backtest

# ── 백테스트 DB 전용 ────────────────────────────────────────────────────────
# DART 재무·공시 수집 (KST 00:05 = UTC 15:05 전날 — 쿼터 리셋 직후)
# ※ 초기 UTC 09:00(KST 18:00)로 잘못 설정 → 쿼터 리셋 전 실행으로 자주 초과
#   UTC 15:05로 수정 (2026-05-21)
5 15 * * *  cd $WORKDIR && $VENV -m ingest.dart_ingest --skip-if-done >> /opt/stock-backtest/logs/dart_ingest.log 2>&1

# pykrx OHLCV (KST 23:00 = UTC 14:00)
0 14 * * *  cd $WORKDIR && $VENV -m ingest.price_ingest >> /opt/stock-backtest/logs/price_ingest.log 2>&1

# 시가총액 수집 (KST 23:30 = UTC 14:30)
30 14 * * *  cd $WORKDIR && $VENV -m ingest.market_cap_ingest >> /opt/stock-backtest/logs/market_cap_ingest.log 2>&1

# DB 백업 (매주 일요일 KST 10:00 = UTC 01:00)
0 1 * * 0  $WORKDIR/scripts/backup_db.sh >> /opt/stock-backtest/logs/backup.log 2>&1
```
...
```bash
# 실제 사용 경로: /opt/stock-backtest/logs/ (프로젝트 디렉토리 내)
