...
- **최소 편입 종목 수 규칙**: `backtest/portfolio.py`에 `MIN_PORTFOLIO_STOCKS=5` 상수가 있으나
  **실제로는 `build_portfolio()`에 적용되지 않음**(코드는 `n==0`일 때만 빈 dict 반환, 5 미만 체크
  없음 — docstring과 실제 동작 불일치, 2026-07-05 코드 확인). 그 결과 G#4=3종목처럼 5 미만 구간도
  실제로 포트폴리오에 편입됨. ① 이 불일치 자체를 고칠지, ② 워크플로우 문서 STEP 7 제안대로 임계값을
  15 등으로 올릴지 둘 다 미확정 — 코드 수정하지 않고 남겨둠.
