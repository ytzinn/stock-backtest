...
−34.14%·−57.08%·−58.12% 같은 숫자는 장식이 아니라 **검사 대상**이다. 산출물이 바뀌면
...
GLOSSARY: tuple[Term, ...] = (
    Term(
        id='strategy_vs_tag',
        term='전략 (화면) = `tag` (코드)',
        one_line='화면이 "전략"이라 부르는 것이 코드·산출물의 `tag` 다. 다른 게 아니라 같은 것이다.',
        aliases=('tag', '태그', 'base_tag', 'ABLATION_CONFIGS', 'F_pbr_ma200'),
        series=(),
        sources=('backtest/ablation.py', 'dashboard/series_view.py'),
        incident='',
        body="""
같은 것을 부르는 두 이름이다.
...
|---|---|---|
| 화면 | **전략** | `F_pbr_ma200` |
| 코드·산출물 | `tag` (= `base_tag`) | `ABLATION_CONFIGS` 의 키 |

화면에서 "태그"라고 쓰지 않는 이유는, 그 말이 **분류 라벨**(글에 붙이는 태그)처럼
읽히기 때문이다. 여기서 가리키는 것은 라벨이 아니라 **필터·랭킹·캘린더 설정 한 벌**,
즉 백테스트를 한 번 돌리는 데 필요한 설정 전체다.

> 전략 이름만으로는 파일을 찾을 수 없다. 같은 전략이라도 종목 수가 다르면 다른
> 산출물이라서, 조회는 `artifact_key`(= 전략 이름 + 종목 수)로 한다.
"""),

    Term(
        id='tag_vs_artifact_key',
        term='`tag` vs `artifact_key`',
        one_line='전략 이름과 파일 조회 키는 다르다. 전략 하나가 여러 종목 수의 결과를 가리킨다.',
        ...
        body="""
- **`tag`** (= `base_tag`) — 전략 설정의 이름. 화면에서는 그냥 **전략**이라 부른다. 예: `F_pbr_ma200`
- **`artifact_key`** — 산출물 파일의 이름. `{tag}_n{종목수}` 규약. 예: `F_pbr_ma200_n13`

**파일 조회는 무조건 `artifact_key` 로 한다.** 전략 이름만으로는 어느 종목 수의 결과인지
...
"""),
    ...
    Term(
        ...
        one_line='같은 전략의 MDD 가 −33.02%(구간·gross)와 −57.82%(일별·net)로 약 25%p 다르다. 둘 다 맞는 값이다.',
...
**두 개의 축이 겹쳐 있다** — ① 구간 vs 일별 ② gross vs net. "구간 −33% / 일별 −58%"
라고만 말하면 두 축을 하나로 뭉개는 것이다. 현행 채택안의 실제 값 셋:

| 기준 | 산출물 필드 | 현행 채택안 |
|---|---|---|
| 구간 · gross | `mdd` (= `endpoint_mdd_gross`) | **−33.02%** |
| 일별 · gross | `daily_mdd_gross` | **−56.78%** |
| **일별 · net** | `net.daily_mdd` | **−57.82%** |

**약 25%p 차이의 대부분은 측정 빈도에서 나온다** (−33.02% → −56.78%, 23.76%p). 거래비용이
더하는 몫은 1%p 남짓(−56.78% → −57.82%, 1.04%p)이다. "비용 때문에 낙폭이 커졌다"고 읽으면
틀린다 — 구간 끝 값만 이으면 구간 **안쪽**의 골짜기가 통째로 안 보이는 것이 원인이다.

> 셋을 나란히 놓을 수 있는 이유: 낙폭 구간(2018-05-14 → 2020-03-19)이 판정 구간(n=18)
> 안에 통째로 들어 있어 **세 값이 절단 전후로 같다.** 구간·gross 는 n=18 에서도 n=20
> 에서도 −33.02% 다. 다른 지표는 그렇지 않으니 이 병치를 다른 지표로 옮기지 마라.

**판정의 SSOT 는 일별 net 이다** (SPEC_13 §9-1). SPEC_10 G5 게이트가 −45% 한계선에
FAIL 인 것도 일별 net −57.82% 기준이다. 다만 **단독 사유는 아니다** — 2026-08 구간
절단 재산출에서 G1 도 FAIL 로 뒤집혔다 (CAGR 15.63% vs 귀무 p95 17.97%).

그런데 **비교표의 MDD·Sharpe 열은 구간 기준으로 통일돼 있다.** 일별 NAV 를 가진 전략이
...
> "부모 전략의 실행"으로 읽으면 이 산출물이 어느 축 소속인지 영영 정해지지 않는다.
> 실제로 그래서 한동안 미배정이었다.
"""),
