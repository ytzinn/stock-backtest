...
포맷: 변수 / 답하는 질문 / 막는 실패 모드 / 판정·종결 상태 / **먹여준 결정(decision history)** /
...
**근본 처방**: `run_ablation` 이 앞으로 만드는 모든 성과 JSON에 `n_stocks` 를 **항상** 기록하게
한다(현재는 기본값일 때 생략). 새로 생기는 파일부터 세 번째 경우가 사라진다. 기존 68개는
재실행 없이는 못 채우니 warning 목록으로 관리한다.
