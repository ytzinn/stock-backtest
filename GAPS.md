...
## Pass 0C — 독립 오라클 + 통합 테스트

**모델**: Fable 5 / effort xhigh (AUDIT_00 §3와 일치 — 세션에서 `/model fable` 전환 확인)

### tests/oracle/ (37개: 31 passed + 5 의도적 실패 + 1 xfail)

O-1(RIM)~O-6(최소종목) 전부 작성. 상수는 `backtest/configs/constants.py`·`backtest.engine.DELISTING_HAIRCUT`에서 import (SSOT, 하드코딩 없음). **의도적 실패 5개 = P0 증거** (xfail 처리하지 않음, AUDIT_01 지시):

| 실패 테스트 | 결함 | TECH_DEBT |
|---|---|---|
| 가중수익률 소비 | weight 무시하고 sum/len | CORR-ENGINE-001 (P0-B) |
| 상폐 조정 순서 독립성 | n 감소로 opt/cons가 순회 순서 의존 — 합성 재현 성공 | CORR-ENGINE-002 (P0-B) |
| turnover 5→20 확장 | 비중 변화 무시 → 0.0 반환 | **CORR-METRIC-001 (P0-A)** |
| CAGR 캘린더일수 | 구간수÷2 관례 (21.00% vs 진실 20.89%, 합성 케이스) | CORR-METRIC-002 (P0-B) |
| Sharpe zero-variance 가드 | 가드는 `returns.std()`, 나눗셈은 `excess.std()` → inf 가능 | CORR-METRIC-003 (P2, **오라클 작성 중 신규 발견**) |

**[검증된 사실] CORR-METRIC-001의 P0-A 확정 근거** (AUDIT_01 O-4의 "반드시 확인" 항목):
turnover는 리포트 전용이 아니라 `engine.py:110-112`에서 `tc = turnover × (COST_SELL+COST_BUY)`로
**거래비용에 직접 입력**된다. Pass 0B tape 스캔(`scripts/audit/turnover_impact_scan.py`) 결과
캡처된 5개 시나리오 전부에서 실측 오염 확인:
- CANONICAL(F_no_r2r3): 2017-04-05 기록 0.25 vs 올바른 정의 1.00, 2020-08-20 기록 0.35 vs 1.00
  → 누적 거래비용 0.952%p 과소계상 (net 수익률이 실제보다 유리하게 보고됨)
- D 계열 4개: 2017-04-05 구간에서 동일 패턴 (+0.476~0.510%p)
- 종목 수 축소 구간(20→5, 20→7)은 완전 교체여서 우연히 일치(1.0 == 1.0)

**[검증된 사실] CORR-ENGINE-002 실데이터 공존 스캔**: 캡처된 5개 tape에서 상폐 종목이 있는
구간(2022-04-05 `066110`, 2023-08-18 `001880`)에 가격결측 종목이 **동시에 존재하지 않음** →
순서 의존 버그는 합성으로 재현됐지만 현재 공표 수치는 영향권 밖 (미캡처 24개 시나리오는 미확인).
P0-A 승격 보류, P0-B 유지.

**[검증된 사실] CAGR 실데이터 영향 추정** (수정 전 영향 기록, AUDIT_01 O-5 지시):
closed 20구간은 2016-04-05→2026-04-03 = 9.99년 vs 관례 10.0년으로 우연히 근접(4월→4월 정렬).
왜곡의 주범은 **열린 구간 #23 포함 시**: n=21 → 10.5년 vs 실제 ≈10.27년 → CANONICAL CAGR
15.27%는 캘린더 기준 ≈15.65%로 약 0.38%p 과소 보고. CORR-ENGINE-003과의 결합(AUDIT_00 §5)
그대로 — closed_period 기준 채택 시 두 문제가 동시에 소거된다는 설계 방침을 실측으로 뒷받침.

### tests/integration/ (30개 전부 통과 — 합성 PostgreSQL 16-alpine, 포트 5434, 실행 후 파기)

I-1~I-6 전부 작성·실행. **룩어헤드 방지 핵심 경로는 전부 계약대로 동작함을 확인**:
- I-1 `available_from <= rebalance_date` 경계 포함/제외 ✅
- I-2 정정공시 리밸런싱 이후 → 원본값 사용 ✅ (단, 아래 PIT-AMEND-001 예외 경로 발견)
- I-3 상폐일 경계(전일/당일/익일) + "가격 끊김 ≠ 상폐" 구분 ✅ (v5.3 회귀 방지 고정)
- I-4 CFS 우선/OFS fallback ✅ (단, **계정 단위** 혼합 발견 — MIX-FSDIV-001)
- I-5 `DISTINCT ON` 최신 연도 선택 + 게이트의 PIT 인지(미공개 연도 판정 미사용) ✅
- I-6 fallback = 법정마감+5일 정확성 (FY/H1/Q1/Q3 전부) + report_type 격리 ✅

**신규 발견 2건** (현행 동작 문서화 테스트로 고정, Pass 1에서 등급 확정):
- **PIT-AMEND-001** [검증된 사실]: `amendment_from > rebalance_date`인데 `original_amount IS NULL`
  이면 CASE ELSE 분기로 정정값이 그대로 사용됨 = 조용한 룩어헤드 경로. 실데이터 발생 건수
  확인 쿼리를 TECH_DEBT.md에 기재 (Pass 1A에서 운영 DB 조회).
- **MIX-FSDIV-001** [검증된 사실]: CFS→OFS fallback이 계정 단위라 한 종목·연도 dict에
  CFS 자본총계와 OFS 당기순이익이 혼합될 수 있음. docstring 계약이 단위를 명시하지 않음.

### 게이트 판정

- `pytest -m "not integration"`: 122 passed + **의도적 실패 5개**(P0 증거, 위 표) + 1 xfail.
  AUDIT_01 게이트 "oracle 중 실패하는 것이 있다면 → 그것이 곧 P0 후보다. 고치지 말고 기록만
  하고 Pass 1로 넘어가라"에 따라 **기록 완료 상태로 게이트 통과 처리**.
- `pytest -m integration`: 30/30 통과.
- oracle/characterization 물리 분리: `tests/oracle/` vs `tests/characterization/` ✅.
...
| CORR-METRIC-003 | compute_sharpe zero-variance 가드가 잘못된 변수 검사 → inf 가능 (Pass 0C 신규) | P2 | 실데이터 발생 불가능 여부만 재확인 |
| PIT-AMEND-001 | 정정 미공개 + original_amount NULL → 정정값 사용 (룩어헤드 경로, Pass 0C 신규) | P0-B 후보 | 운영 DB 발생 건수 쿼리 (TECH_DEBT.md 기재) |
| MIX-FSDIV-001 | CFS→OFS fallback이 계정 단위 → 연결/별도 혼합 dict 가능 (Pass 0C 신규) | P1 | 계약 명문화 + 실데이터 혼합 발생 빈도 |
| FALLBACK-MARGIN-001 | fallback available_from과 리밸런싱일 마진 0~2일, 5개 날짜 경계 일치 + 지연 제출 시 룩어헤드 위험 (Pass 0C 신규) | P1 | 지연 제출 사례 존재 여부 조회 |

**Pass 0 전체 종료 시점의 P0 현황** (Pass 1이 이 목록에서 시작하면 된다 — TECH_DEBT.md가 정본):
- **P0-A 확정 1건**: CORR-METRIC-001 (turnover → 거래비용 입력 확인 + 실측 오염 정량화)
- **P0-B 6건**: CORR-ENGINE-001/002/003, CORR-METRIC-002, CORR-BENCH-001, PIT-AMEND-001(후보)
- 정책 결정 1건: CONTRACT-PF-001 (xfail로 계류)
