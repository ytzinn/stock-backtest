...
#         무효다. C_{k+1}−C_k = (r_{k+1}−C_k)/(k+1) 로 이동폭이 강제 감쇠해 k 두 자리
#         구간에서는 spike 자체가 불가능하다. 랭킹 셔플 귀무에서도 최고점이 k>=10 이면
...
#         k=1 18.78% → k=20 21.15% 로 줄지 않아(전 종목이 같은 저PBR 팩터에 물림)
#         낙폭은 종목 수 축으로 풀리지 않는다 — 별도 과제.
N_STOCKS    = 13
